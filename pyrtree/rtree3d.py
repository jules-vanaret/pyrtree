...
from .rect3d import Rect3D, union_all, NullRect

class RTree3D(object):
    def __init__(self):
        self.count = 0
        self.stats = { 
            "overflow_f" : 0,
        ...
        self.cursor = _NodeCursor3D.create(self, NullRect)
...
class _NodeCursor3D(object):
    ...
    def create(cls, rooto, rect):
        ...
        retv = _NodeCursor3D(rooto,idx,rect,0,0)
    ...
    def create_with_children(cls, children, rooto):
        ...
        nr = Rect3D(rect.x,rect.y,rect.z,rect.xx,rect.yy,rect.zz)
        assert(not rect.swapped_x)
        nc = _NodeCursor3D.create(rooto,rect)
    ...
    def create_leaf(cls, rooto, leaf_obj, leaf_rect):
        rect = Rect3D(leaf_rect.x,leaf_rect.y,leaf_rect.z,leaf_rect.xx,leaf_rect.yy,leaf_rect.zz)
        rect.swapped_x = True # Mark as leaf by setting the xswap flag.
        res = _NodeCursor3D.create(rooto, rect)
        idx = res.index
    ...
    def lift(self):
        return _NodeCursor3D(self.root,
                           self.index,
                           self.rect, 
                           self.first_child,
                           self.next_sibling)
    ...
    def _become(self, index):
        ...
        if (x == 0.0 and y == 0.0 and z == 0.0 and xx == 0.0 and yy == 0.0 and zz == 0.0): 
            ...
            self.rect = Rect3D(x,y,z,xx,yy,zz)
    ...
    def get_first_child(self):
        fc = self.first_child
        c = _NodeCursor3D(self.root,0,NullRect,0,0)
    ...
    def insert(self, leafo, leafrect):
        ...
        while True:
            if self.holds_leaves():
                ...
                self._insert_child(_NodeCursor3D.create_leaf(self.root,leafo,leafrect))
    ...
    def _balance(self):
        ...
        nodes = [ _NodeCursor3D.create_with_children(c,self.root) for c in bestcluster if len(c) > 0]
